...
public class UntrustCommandLogic {
    ...
    public static int execute(CommandSourceStack source, ServerPlayer player, ServerPlayer target) {
        ...
        if (playerUUID.equals(targetUUID)) {
            source.sendFailure(Component.literal("You cannot untrust yourself."));
            return 0;
        }
        ...
        if (playerData.removeTrustedPlayer(targetUUID)) {
            ...
            source.sendSuccess(() -> Component.literal("You no longer trust ")
        ...
        source.sendFailure(Component.empty()
                .append(target.getDisplayName())
                .append(" was not on your trust list."));
        return 0;
    }
}
